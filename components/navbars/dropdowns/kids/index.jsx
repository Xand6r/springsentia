...
export default function Index({ open }) {
  ...
  return (
    <div className={`${generalStyles.nav_dropdown} ${styles.kids_dropdown}`}>
      <section
      ...
      >
        ...
        <div>
          ...
          {Object.keys(DEFAULT_STRUCTURE).map((oneKey, index) => (
            <h6
              key={index + oneKey}
              className={oneKey === activeSection && generalStyles.__active}
              onClick={
      ...
      {activeSection ? (
        ...
        >
          ...
          <div className={generalStyles.section__divider} />
          <div>
            {DEFAULT_STRUCTURE[activeSection].map((oneKey, index) => (
              <h6
                key={index + oneKey.name}
                className={
                  oneKey.name === activeSubsection && generalStyles.__active
                }
                onClick={() =>
      ...
      {activeSubsection &&
      DEFAULT_STRUCTURE[activeSection].find(
        ({ name }) => name === activeSubsection
      )?.image ? (
